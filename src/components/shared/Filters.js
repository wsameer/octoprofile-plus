...
const Filters = ({ ...args }) => {
  return (
    <ButtonToolbar
      ...
      <DropdownButton className="pr-3" id="dropdown-item-button" title={args.sortType}>
        {args.sortTypes.map((s, i) => (
          <Dropdown.Item key={i} as="button" onClick={() => args.changeSortType(s)}>{s}</Dropdown.Item>
        ))}
      </DropdownButton>
      ...
      <ButtonToolbar aria-label="Filter controls for the repos">
        <ButtonGroup aria-label="view as group">
          <Button variant="secondary"
            className={args.viewType === args.viewTypes[1] ? 'active' : ''}
            onClick={() => args.changeViewType(args.viewTypes[1])}>
            <i className="fa fa-list-ul" aria-hidden="true"></i>
          </Button>
          <Button variant="secondary"
            className={args.viewType === args.viewTypes[0] ? 'active' : ''}
