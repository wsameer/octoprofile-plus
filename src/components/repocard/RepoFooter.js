...
const RepoFooter = ({ ...args }) => {
  return (
    <>
      ...
      <a href={args.forksUrl} className="pinned-item-meta muted-link pr-3">
        <i className="fa fa-code-fork mr-1" aria-hidden="true"></i>
        {args.forksCount.toLocaleString()}
      </a>
      {args.updatedAt && <span className="float-right">Updated {relativeTimeConvertor(args.updatedAt)}</span>}
      {args.size && <span className="repo-size float-right">{args.size.toLocaleString()} KB</span>}
    </>
  )
}
